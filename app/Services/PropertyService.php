...
class PropertyService
{
    ...
    public function uploadEngFile(string $title, UploadedFile $file, object $model, bool $delete = false)
    {

        if ($delete) {
            if (File::isFile(public_path('investment/property/' . $model->en_file))) {
                File::delete(public_path('investment/property/' . $model->en_file));
            }
            if (File::isFile(public_path('investment/property/thumbs/' . $model->en_file))) {
                File::delete(public_path('investment/property/thumbs/' . $model->en_file));
            }
            if (File::isFile(public_path('investment/property/list/' . $model->en_file))) {
                File::delete(public_path('investment/property/list/' . $model->en_file));
            }

            // WebP
            if (File::isFile(public_path('investment/property/webp/' . $model->en_file_webp))) {
                File::delete(public_path('investment/property/webp/' . $model->en_file_webp));
            }
            if (File::isFile(public_path('investment/property/thumbs/webp/' . $model->en_file_webp))) {
                File::delete(public_path('investment/property/thumbs/webp/' . $model->en_file_webp));
            }
            if (File::isFile(public_path('investment/property/list/webp/' . $model->en_file_webp))) {
                File::delete(public_path('investment/property/list/webp/' . $model->en_file_webp));
            }
        }

        $name = date('His').'_en_'.Str::slug($title).'.' . $file->getClientOriginalExtension();
        $name_webp = date('His').'_en_'.Str::slug($title).'.webp';
        $file->storeAs('property', $name, 'investment_uploads');
    ...
    public function uploadEngPdf(string $title, UploadedFile $file, object $model, bool $delete = false)
    {

        if ($delete) {
            if (File::isFile(public_path('investment/property/pdf/' . $model->en_file_pdf))) {
                File::delete(public_path('investment/property/pdf/' . $model->en_file_pdf));
            }
        }

        $name = date('His').'_en_'.Str::slug($title).'.' . $file->getClientOriginalExtension();
        $file->storeAs('property/pdf', $name, 'investment_uploads');
        $model->update(['en_file_pdf' => $name]);
    }
}
